...
class SegmentTree:
    ...
    def __init__(self, data):
        # node: (value, left_node_index, right_node_index)
    ...
    def _update(self, node_index, data_index, update_value, left_data_index, right_data_index):
        ...
        if left_data_index == right_data_index:
            ...
            if data_index <= mid_data_index:
                left_value = self._update(
                    left_node_index, data_index, update_value, left_data_index, mid_data_index)
                right_value = self.nodes[right_node_index]
            else:
                left_value = self.nodes[left_node_index]
                right_value = self._update(
                    right_node_index, data_index, update_value, mid_data_index + 1, right_data_index)
            (left_lc, left_ll, left_pc, left_pl, left_sc, left_sl) = left_value
            (right_lc, right_ll, right_pc, right_pl,
             right_sc, right_sl) = right_value
