...
class Solution:
    ...
    def maximum_length(self, nums: list[int], k: int) -> int:
        """
        good_sequences[i] stands for the maximum length of
        a good sequence with at most i different neighbour.

        dp[i][x] stands for the maximum length of a good
        sequence with at most i different neighbour and ends
        with number x.

        So dp[i][a] = max(dp[i][a] + 1, good_sequences[i - 1] + 1)
        """
